...
def generate_lalo_view8():
    LATITUDE = 2
    LONGITUDE = 4
    views = np.zeros((LATITUDE,LONGITUDE,3))
    for i in range(LONGITUDE):
        for j in range(LATITUDE):
            theta = (i/LONGITUDE)*2*np.pi + np.pi/4
            phi = (j/LATITUDE)*np.pi + np.pi/4
            ...
            views[j, i, 1] = y
            views[j, i, 2] = z
    return torch.from_numpy(views.reshape(-1,3).astype(np.float32))

def generate_lalo_view12():
    LATITUDE = 3
    LONGITUDE = 4
    views = np.zeros((LATITUDE,LONGITUDE,3))
    for i in range(LONGITUDE):
